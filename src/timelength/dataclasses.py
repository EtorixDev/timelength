...
class Scale:
    ...
    def __repr__(self) -> str:
        ...
        def tuple_to_str(tup: tuple[str, ...]) -> str:
...
class Numeral:
    ...
    - terms: `tuple[str, ...] = ()` — A tuple of terms associated with the numeral.
    - enabled: `bool = True` — Whether the numeral is enabled.
    ...
    def valid(self) -> bool:
        ...
        return bool(self.name and self.type and self.type is not NumeralType.NONE and self.value is not None and self.terms)  # pyright: ignore[reportUnnecessaryComparison]
...
class ParsedTimeLength:
    ...
    def __repr__(self) -> str:
        ...
        def tuple_to_str(tup: tuple[object, ...]) -> str:
            return "(" + ", ".join([tuple_to_str(cast(tuple[object, ...], term)) if isinstance(term, tuple) else repr(term) if isinstance(term, (Scale, FailureFlags)) else json.dumps(term) for term in tup]) + ")"
...
class ParserSettings:
    ...
    def __repr__(self) -> str:
        """
        Return a string representation of the parser settings.

        #### Returns
        - A representation containing every setting and its current value.
        """
