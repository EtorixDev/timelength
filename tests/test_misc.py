...
def test_parsedtimelength_delta(tl_notstrict: TimeLength) -> None:
    tl_notstrict.content = "5 seconds"
    tl_notstrict.parse()
...
def test_parsedtimelength_strings(tl_notstrict: TimeLength) -> None:
    tl_notstrict.content = "5 seconds"
    tl_notstrict.parse()
...
def test_parsedtimelength_oob(tl_notstrict: TimeLength) -> None:
...
def test_error_messages() -> None:
    ...
    assert str(InvalidLocaleError(loc := English())) == f"The configuration for {str(loc)!r} is invalid."
    assert str(InvalidLocaleError(loc, "Testing error.")) == "Testing error."
    assert str(InvalidParserError(loc)) == f"{str(loc)!r} is missing a valid parser function."
    assert str(InvalidParserError(loc, "Testing error.")) == "Testing error."


def test_locale_symbols() -> None:
    locale = English()
    locale._parser = None  # pyright: ignore[reportPrivateUsage]
...
def test_locale_properties_functions() -> None:
    locale = English()
    scales = locale.scales
    locale.second.enabled = False

    assert locale.base_scale is locale.millisecond
...
def test_load_config(mocker: MockerFixture) -> None:
    ...
    with pytest.raises(InvalidLocaleError):
        mocker.patch("builtins.open", mocker.mock_open(read_data="{}"))  # pyright: ignore[reportUnknownMemberType]
        Locale("english.json")


def test_load_parser(mocker: MockerFixture) -> None:
    ...
    with pytest.raises(InvalidLocaleError) as excinfo5:
        mocker.patch("builtins.open", mocker.mock_open(read_data='{"parser_file": "parser_fake.py"}'))  # pyright: ignore[reportUnknownMemberType]
        Locale("english.json")

    assert isinstance(excinfo5.value.__cause__, FileNotFoundError)


def test_load_flags(mocker: MockerFixture) -> None:
    eng = Locale("english.json")

    with pytest.raises(InvalidLocaleError) as excinfo1:
        mocker.patch("timelength.locales.Locale._get_config_or_raise", return_value=["FakeFlag"])
        eng._load_flags(None)  # pyright: ignore[reportPrivateUsage]

    assert isinstance(excinfo1.value.__cause__, KeyError)


def test_load_settings(mocker: MockerFixture) -> None:
    eng = Locale("english.json")

    with pytest.raises(InvalidLocaleError) as excinfo1:
        mocker.patch("timelength.locales.Locale._get_config_or_raise", return_value={"FakeSetting": "FakeValue"})
        eng._load_settings(None)  # pyright: ignore[reportPrivateUsage]
...
def test_load_scales() -> None:
    eng = Locale("english.json")
    empty_scale_data: dict[str, dict[str, Any]] = {}

    with pytest.raises(InvalidLocaleError) as excinfo1:
        eng._load_scales(empty_scale_data)  # pyright: ignore[reportPrivateUsage]
    ...
    eng._load_scales({**eng._scales_json, **custom_scale_data})  # pyright: ignore[reportPrivateUsage]
    ...
    with pytest.raises(InvalidScaleError) as excinfo2:
        eng._load_scales({**eng._scales_json, **no_name_data})  # pyright: ignore[reportPrivateUsage]
    ...
    with pytest.raises(InvalidScaleError) as excinfo3:
        eng._load_scales({**eng._scales_json, **not_enough_data})  # pyright: ignore[reportPrivateUsage]
    ...
    with pytest.raises(InvalidScaleError) as excinfo4:
        eng._load_scales({**eng._scales_json, **invalid_value_data})  # pyright: ignore[reportPrivateUsage]
    ...
    with pytest.raises(InvalidScaleError) as excinfo5:
        eng._load_scales({**eng._scales_json, **invalid_type_data})  # pyright: ignore[reportPrivateUsage]
    ...
    with pytest.raises(InvalidScaleError) as excinfo6:
        eng._load_scales({**eng._scales_json, **too_many_keys_data})  # pyright: ignore[reportPrivateUsage]
    ...
    with pytest.raises(NoValidScalesError):
        eng._validate_scales()  # pyright: ignore[reportPrivateUsage]


def test_load_numerals() -> None:
    eng = Locale("english.json")
    no_name_data = {"": {"type": "DIGIT", "value": 1, "terms": ["one"]}}

    with pytest.raises(InvalidNumeralError) as excinfo1:
        eng._load_numerals(no_name_data)  # pyright: ignore[reportPrivateUsage]
    ...
    with pytest.raises(InvalidNumeralError) as excinfo2:
        eng._load_numerals(not_enough_data)  # pyright: ignore[reportPrivateUsage]
    ...
    with pytest.raises(InvalidNumeralError) as excinfo3:
        eng._load_numerals(invalid_value_data)  # pyright: ignore[reportPrivateUsage]

    assert isinstance(excinfo3.value.__cause__, ValueError)


def test_get_config_or_raise() -> None:
    eng = Locale("english.json")
    eng._config = {"connectors": 1}  # pyright: ignore[reportPrivateUsage]

    with pytest.raises(InvalidLocaleError) as excinfo1:
        eng._get_config_or_raise("connectors", list)  # pyright: ignore[reportPrivateUsage]

    assert str(excinfo1.value) == "'connectors' key in config is not of type 'list'."

    with pytest.raises(InvalidLocaleError) as excinfo2:
        eng._get_config_or_raise("fake_key", list)  # pyright: ignore[reportPrivateUsage]

    assert str(excinfo2.value) == "No 'fake_key' key found in config."


def test_locale_str_repr() -> None:
...
def test_timelength() -> None:
    with pytest.raises(NotALocaleError):
        tl = TimeLength(content="5 seconds", locale=Buffer())  # pyright: ignore[reportArgumentType]

    tl = TimeLength(content="3 days, 2 hours, 1 minute, 30.5 seconds", locale=English())

    assert str(tl) == "3 Days, 02:01:30.5"
    assert tl._convert_to_hhmmss(tl.result.seconds) == "3 Days, 02:01:30.5"  # pyright: ignore[reportPrivateUsage]

    tl.content = "5 minutos"
    tl.parse(guess_locale=True)

    assert isinstance(tl.locale, Spanish)
    assert tl._convert_to_hhmmss(tl.result.seconds) == "00:05:00"  # pyright: ignore[reportPrivateUsage]
...
def test_timelength_conversions() -> None:
    ...
    assert tl.hence(base=naive_base) == (naive_base + timedelta(minutes=5))
    ...
    tl.content = tl._convert_to_hhmmss(datetime(9999, 12, 31, 23, 59, 59, 999999, tzinfo=timezone.utc).timestamp())  # pyright: ignore[reportPrivateUsage]
    tl.parse()
...
def test_timelength_dunder_methods() -> None:
    ...
    assert tl_5_minutes.__add__(buffer) is NotImplemented  # pyright: ignore[reportArgumentType]
    assert tl_5_minutes.__add__(dt_now) is NotImplemented  # pyright: ignore[reportArgumentType]

    for unsupported in (buffer, dt_now):
        with pytest.raises(TypeError):
            _ = tl_5_minutes + unsupported  # pyright: ignore[reportOperatorIssue, reportUnknownVariableType]
    ...
    assert tl_5_minutes.__radd__(buffer) is NotImplemented  # pyright: ignore[reportArgumentType]
    assert tl_5_minutes.__radd__(hundred) is NotImplemented  # pyright: ignore[reportArgumentType]

    for unsupported in (buffer, hundred):
        with pytest.raises(TypeError):
            _ = unsupported + tl_5_minutes  # pyright: ignore[reportOperatorIssue, reportUnknownVariableType]
    ...
    assert tl_5_minutes.__sub__(buffer) is NotImplemented  # pyright: ignore[reportArgumentType]
    assert tl_5_minutes.__sub__(dt_now) is NotImplemented  # pyright: ignore[reportArgumentType]
    ...
    assert isinstance((res := dt_now - tl_5_minutes), datetime) and (res - dt_now).total_seconds() == -300
    assert isinstance((res := td_5_minutes - tl_5_minutes), timedelta) and res.total_seconds() == 300 - 300
    assert tl_5_minutes.__rsub__(buffer) is NotImplemented  # pyright: ignore[reportArgumentType]
    assert tl_5_minutes.__rsub__(hundred) is NotImplemented  # pyright: ignore[reportArgumentType]
    ...
    assert tl_5_minutes.__mul__(buffer) is NotImplemented  # pyright: ignore[reportArgumentType]
    assert tl_5_minutes.__mul__(dt_now) is NotImplemented  # pyright: ignore[reportArgumentType]
    assert tl_5_minutes.__mul__(tl_10_minutes) is NotImplemented  # pyright: ignore[reportArgumentType]
    assert tl_5_minutes.__mul__(td_10_minutes) is NotImplemented  # pyright: ignore[reportArgumentType]
    ...
    assert isinstance((res := tl_5_minutes / hundred), TimeLength) and res.result.seconds == 300 / 100
    assert tl_5_minutes.__truediv__(buffer) is NotImplemented  # pyright: ignore[reportArgumentType]
    assert tl_5_minutes.__truediv__(dt_now) is NotImplemented  # pyright: ignore[reportArgumentType]
    ...
    assert tl_5_minutes.__rtruediv__(buffer) is NotImplemented  # pyright: ignore[reportArgumentType]
    assert tl_5_minutes.__rtruediv__(hundred) is NotImplemented  # pyright: ignore[reportArgumentType]
    assert tl_5_minutes.__rtruediv__(dt_now) is NotImplemented  # pyright: ignore[reportArgumentType]
    ...
    with pytest.raises(PotentialTimeDeltaError):
        _ = timedelta.max + tl_5_minutes

    with pytest.raises(PotentialTimeDeltaError):
        _ = timedelta.min - tl_5_minutes
    ...
    assert isinstance((res := tl_5_minutes // hundred), TimeLength) and res.result.seconds == 300 // 100
    assert tl_5_minutes.__floordiv__(buffer) is NotImplemented  # pyright: ignore[reportArgumentType]
    assert tl_5_minutes.__floordiv__(dt_now) is NotImplemented  # pyright: ignore[reportArgumentType]
    ...
    assert tl_5_minutes.__rfloordiv__(buffer) is NotImplemented  # pyright: ignore[reportArgumentType]
    assert tl_5_minutes.__rfloordiv__(hundred) is NotImplemented  # pyright: ignore[reportArgumentType]
    assert tl_5_minutes.__rfloordiv__(dt_now) is NotImplemented  # pyright: ignore[reportArgumentType]
    ...
    assert isinstance((res := tl_5_minutes % td_10_minutes), TimeLength) and res.result.seconds == 300 % 600
    assert isinstance((res := tl_5_minutes % hundred), TimeLength) and res.result.seconds == 300 % 100
    assert tl_5_minutes.__mod__(buffer) is NotImplemented  # pyright: ignore[reportArgumentType]
    assert tl_5_minutes.__mod__(dt_now) is NotImplemented  # pyright: ignore[reportArgumentType]
    ...
    assert tl_5_minutes.__rmod__(buffer) is NotImplemented  # pyright: ignore[reportArgumentType]
    assert tl_5_minutes.__rmod__(hundred) is NotImplemented  # pyright: ignore[reportArgumentType]
    assert tl_5_minutes.__rmod__(dt_now) is NotImplemented  # pyright: ignore[reportArgumentType]
    ...
    tl_out_of_bounds = TimeLength(content="999,999 centuries", locale=English())

    with pytest.raises(ParsedTimeDeltaError):
        _ = timedelta() + tl_out_of_bounds
    ...
    assert tl_5_minutes.__divmod__(buffer) is NotImplemented  # pyright: ignore[reportArgumentType]
    assert tl_5_minutes.__divmod__(dt_now) is NotImplemented  # pyright: ignore[reportArgumentType]

    for unsupported in (buffer, dt_now):
        with pytest.raises(TypeError):
            _ = divmod(tl_5_minutes, unsupported)  # pyright: ignore[reportArgumentType, reportCallIssue, reportUnknownVariableType]
    ...
    assert tl_5_minutes.__rdivmod__(buffer) is NotImplemented  # pyright: ignore[reportArgumentType]
    assert tl_5_minutes.__rdivmod__(hundred) is NotImplemented  # pyright: ignore[reportArgumentType]
    assert tl_5_minutes.__rdivmod__(dt_now) is NotImplemented  # pyright: ignore[reportArgumentType]

    for unsupported in (buffer, hundred, dt_now):
        with pytest.raises(TypeError):
            _ = divmod(unsupported, tl_5_minutes)  # pyright: ignore[reportArgumentType, reportCallIssue, reportUnknownVariableType]
    ...
    assert tl_5_seconds.__pow__(buffer) is NotImplemented  # pyright: ignore[reportArgumentType]
    assert tl_5_seconds.__pow__(dt_now) is NotImplemented  # pyright: ignore[reportArgumentType]
    assert tl_5_seconds.__pow__(td_10_minutes) is NotImplemented  # pyright: ignore[reportArgumentType]
    assert tl_5_seconds.__pow__(tl_10_seconds) is NotImplemented  # pyright: ignore[reportArgumentType]

    for unsupported in (buffer, dt_now, td_10_minutes, tl_10_seconds):
        with pytest.raises(TypeError):
            _ = tl_5_seconds**unsupported  # pyright: ignore[reportOperatorIssue, reportUnknownVariableType]
    ...
    for unsupported in (buffer, hundred, dt_now, td_5_seconds, tl_10_seconds):
        with pytest.raises(TypeError):
            _ = unsupported**tl_5_seconds  # pyright: ignore[reportOperatorIssue, reportUnknownVariableType]
    ...
    with pytest.raises(TypeError):
        _ = ~tl_5_seconds  # pyright: ignore[reportOperatorIssue, reportUnknownVariableType]

    assert tl_10_seconds > tl_5_seconds
    assert tl_10_seconds > td_5_seconds
    assert tl_10_seconds > 5
    assert tl_10_seconds.__gt__(buffer) is NotImplemented  # pyright: ignore[reportArgumentType]
    assert tl_10_seconds.__gt__(dt_now) is NotImplemented  # pyright: ignore[reportArgumentType]

    for unsupported in (buffer, dt_now):
        with pytest.raises(TypeError):
            _ = tl_10_seconds > unsupported  # pyright: ignore[reportOperatorIssue, reportUnknownVariableType]
    ...
    assert tl_10_seconds.__ge__(buffer) is NotImplemented  # pyright: ignore[reportArgumentType]
    assert tl_10_seconds.__ge__(dt_now) is NotImplemented  # pyright: ignore[reportArgumentType]
    ...
    assert tl_5_seconds.__lt__(buffer) is NotImplemented  # pyright: ignore[reportArgumentType]
    assert tl_5_seconds.__lt__(dt_now) is NotImplemented  # pyright: ignore[reportArgumentType]
    ...
    assert tl_5_seconds.__le__(buffer) is NotImplemented  # pyright: ignore[reportArgumentType]
    assert tl_5_seconds.__le__(dt_now) is NotImplemented  # pyright: ignore[reportArgumentType]
